  ...
  registerTools(server, [
    {
      ...
      inputSchema: {
        ...
        useCustomProps: z.boolean().optional().describe("Export Blender custom properties as FBX user-data — REQUIRED for UE5 sockets (SOCKET_*) to survive the round trip."),
    ...
    },
    {
      ...
      inputSchema: {
        ...
        useArmatureDeformOnly: z.boolean().optional().describe("If true, drop non-deform bones (IK, controls). Set false to preserve full skeleton."),
        useCustomProps: z.boolean().optional().describe("Export Blender custom properties — required for UE5 socket metadata to round-trip."),
        frameStart: z.number().int().optional().describe("Temporarily override scene.frame_start during this export only (restored after). Use to export a single-frame slice."),
        frameEnd: z.number().int().optional().describe("Temporarily override scene.frame_end during this export only (restored after)."),
        pathMode: PathMode.optional(),
      },
